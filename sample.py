...
out_dir = 'out-levels' # ignored if init_from is not 'resume'
...
max_new_tokens = rows*cols
start = difficulty + str(rows) + str(cols)
print("Start: ", start)
...
with torch.no_grad():
    with ctx:
        for k in range(num_samples):
            ...
            level= parse_encoding(decode(y[0].tolist()))
